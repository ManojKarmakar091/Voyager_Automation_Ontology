...
describe('Ontology Name_Individual API Testing', () => {
    ...
    it('158530 Enabler - Validate the Suggest and Retrieve class API  with "searchType= contains","queryText": "Strut" for page 1 size 10, @Regression, @Smoke', async () => {

        logger.logger().info('POST CALL to Class Search  with "searchType= sratrswith" for page 1 size 10')
        var ClassSearchPayloadData = require("../testdata/Ontology/Class_Search")
        const classSearchPostUrl = ontologyConfig.hostname_Ontology + ontologyConfig.ClassSearch_EndPoint
        ...
        await selectDAOModule.selectQueryById_SearchClassOntologyManagementDB(ClassSearch.data.data.result[1].name).then(async function (items) {
            ...
            items.forEach(item => {
                ...
                expect(item.name).toBe(ClassSearch.data.data.result[1].name);

                //-------------------Assert Search Class of descendants "queryText": "Strut" from Ontology DB------------------------------------
                expect(item.descendants[0].name).toBe(ClassSearch.data.data.result[1].descendants[0].name);
